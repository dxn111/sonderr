...
assert.match(html, /id="studiosBtn"[\s\S]*?Sonderr Studios/);
assert.match(html, /id="studiosHome"[\s\S]*?Developer Program[\s\S]*?Bounty Program/);
...
assert.equal(plugins[1].id, "code-review");
...
assert.match(ui, /\$\("studiosBtn"\)\.onclick = \(\) => openStudios\(\)/);
assert.match(ui, /surface: state\.surface/);
assert.match(ui, /<summary>Read more<\/summary>/);
assert.match(ui, /Release date/);
assert.match(ui, /function usePlugin\(pluginId\)[\s\S]*?state\.activePluginId = plugin\.id[\s\S]*?setMode\(plugin\.id === "code-review" \? "ask" : "build"\)/);
...
assert.match(app, /session\.surface === "studios"/);
